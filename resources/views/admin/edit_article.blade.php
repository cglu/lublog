...
foreach ($categories_list as $categories) {
    echo "<option value=\"{$categories->id}\"" . (old('category_id', $article->category_id) == $categories->id ? "selected='1'" : "") . ">{$categories->name}</option>";
}
?>
	...
	<div class="form-group">
		<label for="exampleInputPassword1">关键字</label> <input type="text"
			...
			value="{{old('keywords',$article->keywords)}}"> <span id="helpBlock"
			class="help-block"><?php echo $errors->first('keywords');?></span>
	</div>
